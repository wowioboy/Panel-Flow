...
$query = "SELECT a.AdTag300
          from projects as p
		  INNER JOIN comic_settings as ps on ps.ComicID=p.ProjectID
		  INNER JOIN ad_tags as a on ps.tag_id=a.id
		  where p.SafeFolder='$Project'";
$AdTag = $DB->queryUniqueValue($query);
$DB->close();
if ($AdTag == '') {*/
	$AdTag = '<script language=javascript src="http://ads.addynamix.com/creative/2-2127755-2j?"></script>
<noscript language=javascript><a href="http://ads.addynamix.com/click/2-2127755-2"><img src="http://ads.addynamix.com/creative/2-2127755-2" border=0></a></noscript>';
 /*
<!-- AD TAG BEGINS: WeVolt(gr.wevolt) / pub1 / 300x250 -->
