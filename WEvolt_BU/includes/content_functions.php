...
include_once($_SERVER['DOCUMENT_ROOT'].'/classes/defineThis.php');
	 ...
	 if (!function_exists('insertUpdate')) {
			...
			if (($ContentType == 'pages') || ($ContentType == 'pencils')|| ($ContentType == 'inks')|| ($ContentType == 'letters')|| ($ContentType == 'colors')|| ($ContentType == 'script')|| ($ContentType == 'extras')){
				...
				//	print $query.'<br/>';
					...
					$query = "SELECT t.*, u.username, p.Thumb from pf_forum_topics as t
							 left JOIN projects as p on p.ProjectID=t.ProjectID and p.ProjectID != ''
							 join users as u on t.UserID=u.encryptid
							  where t.EncryptID='$ContentID'";
					...
					if ($UpdateType == 'project')
						$ContentLink = 'http://'.$_SERVER['SERVER_NAME'].'/forum/index.php?project='.$SafeFolder.'a=read&topic='.$ContentArray->ID;
					else
						$ContentLink = 'http://www.wevolt.com/forum/index.php?user='.trim($ContentArray->username).'&a=read&topic='.$ContentArray->ID;
						
					$Output = insertUpdate('forum topic', 'started', $ContentID, $UpdateType, $_SESSION['userid'], $ContentLink,$UpdateContentID,$ContentArray->Subject);
					
			}else if ($ContentType == 'forum post') {
				
					$query = "SELECT m.*, p.Thumb from pf_forum_messages as m
							  left JOIN projects as p on p.ProjectID=m.ProjectID and p.ProjectID != ''
							  
							  where (m.ID='$ContentID' or m.EncryptID='$ContentID')";
				...
				//	print $query;
					if ($UpdateType == 'project')
						$ContentLink = 'http://'.$_SERVER['SERVER_NAME'].'/forum/index.php?project='.$SafeFolder.'&topic='.$ContentArray->TopicID.'&msg='.$ContentArray->ID;
					else
						$ContentLink = 'http://www.wevolt.com/forum/index.php?user='.trim($ContentArray->username).'&a=read&topic='.$ContentArray->TopicID.'&msg='.$ContentArray->ID;
					//	print $query.'<br/>';
					...
					$query = "SELECT b.*,p.Thumb from pf_forum_boards as b
							  left JOIN projects as p on p.ProjectID=b.ProjectID and p.ProjectID != ''
							  where (b.EncryptID='$ContentID' or b.ID='$ContentID')";
					...
					//print $query.'<br/>';
					if ($UpdateType == 'project')
						$ContentLink = 'http://'.$_SERVER['SERVER_NAME'].'/forum/index.php?project='.$SafeFolder.'&a=read&board='.$ContentArray->ID;
					else
						$ContentLink = 'http://www.wevolt.com/forum/index.php?user='.trim($ContentArray->username).'&a=read&board='.$ContentArray->ID;
						
					$output = insertUpdate('forum board', 'created', $ContentID, 	$UpdateType, $UserID,$ContentLink,$UpdateContentID,$ContentArray->Title);
