	...
	class user {		
			...
			public function getUserInfo($UserID) {
					...
					$query = "select us.*, u.overage, u.email, u.avatar, u.username,
							  (select count(*) from pf_subscriptions as s where s.UserID=us.UserID and s.Status='active' and (s.SubscriptionType='hosted' or s.SubscriptionType='hosted_no_ads') and ((s.Temp=0) or ((s.Temp=1) and (s.Expires>=NOW())))) as IsPro,
		...
		public function getUserProjects($UserID) {
					...
					$query = "select p.*, (SELECT count(*) from follows as f where f.follow_id=p.ProjectID and f.type='project') as TotalFans
					           from projects as p
							   where (p.UserID='$UserID' or p.CreatorID='$UserID') and p.Hosted=1 and p.Published=1 order by p.title";
							   
    				$db->query($query);
					
					while ($project = $db->fetchNextObject()) {
						
						echo '<table border="0" cellspacing="0" cellpadding="0" width="'.($_SESSION['contentwidth']-200).'">
          			<tr>
                        <td  id="updateBox_TL"></td>
                        <td width="'.( $_SESSION['contentwidth']-216).'" id="updateBox_T"></td>
                    ...
                    <tr>
						...
						echo '<tr><td colspan="3"><span class="large_dark_blue"><b>'.$project->title.'</b></span><div class="spacer"></div><span class="blue_cell_title">GENRE:'.$project->genre.'</span></td></tr>';
						echo '<tr>';
							echo '<td width="100" valign="top"><a href="http://www.wevolt.com/'.$project->SafeFolder.'/"><img src="http://www.wevolt.com'.$project->thumb.'" width="90" height="90" class="navbuttons"></a></td>';
							echo '<td valign="top" class="blue_cell_title">LEVEL:'.$project->Ranking.'<div class="spacer"></div>Project Type:'.$project->ProjectType.'<div class="spacer"></div>Number of Pages:'.$project->pages.'</td>';
							echo '<td valign="top" class="blue_cell_title">Number of Fans:'.$project->TotalFans.'<div class="spacer"></div>Created Date:'.$project->createdate.'<div class="spacer"></div>Last Updated:'.$project->PagesUpdated.'</td>';
						echo '</tr>';
						echo '<tr>';
       ...
       echo '<div class="spacer"></div>';
			...
			public function getExcites($UserID) {
					$db = new DB(PANELDB, PANELDBHOST, PANELDBUSER, PANELDBPASS);
					$query = "select distinct e.Blurb, e.ContentID, e.ContentType, e.CreatedDate, e.Comment, e.Target, e.Link, u.avatar, u.username, p.thumb as ProjectThumb, e.thumb as ExciteThumb
									...
									while ($ExciteArray = $db->FetchNextObject()) {
											if ($ExciteArray->ContentType == 'user') 
												$Thumb = $ExciteArray->avatar;
											else if (($ExciteArray->ContentType =='comic') || ($ExciteArray->ContentType =='writing')) 
												$Thumb = "http://www.wevolt.com".$ExciteArray->ProjectThumb;
											else  
												$Thumb = $ExciteArray->ExciteThumb;
												
											echo"<table border='0' cellspacing='0' cellpadding='0' width='600'><tr>
												<td id=\"updateBox_TL\"></td>
											...
											echo "</td><td class=\"updateboxcontent\"></td>
