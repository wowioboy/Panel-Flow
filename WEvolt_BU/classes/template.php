	...
	class template {
		...
		function __construct($ProjectID,$CurrentTheme,$SkinCode,$TemplateCode, $MenuCustom) {
							...
							$query =  "SELECT ts.*,tsk.*, t.HTMLCode, t.MenuLayout, p.SafeFolder
								...
								inner join projects as p on p.ProjectID='$ProjectID'
							...
							if ($TopImageHeight == '')
								$TopImageHeight = $TopRightCornerWidth;
							...
							if ($BottomImageHeight == '')
								$BottomImageHeight = $BottomRightCornerWidth;
		...
		public function getTemplateSection($Section) {
					...
					switch ($Section) {
							case ('Header'):
								...
								$_SESSION['headerrollover'] = $this->TemplateArray->HeaderRollover;
						...
						if ($TargetImage != ''){
							$Image = '<img src="/'.$TargetImage.'"';
							if ($TargetRollover != '') 
								$Image .= 'onmouseover="this.src=\'http://www.wevolt.com/'.$TargetRollover.'\';" onmouseout="this.src=\'http://www.wevolt.com/'.$TargetImage.'\';"';
		...
		public function insertTemplateContent($ContentString) {
				...
				if (($this->AccessType == 'public') || (($this->AccessType == 'fans') && (($this->IsFollowing == 1)||($this->IsProjectSuperFan == 1)))|| (($this->AccessType == 'superfans') && ($this->IsProjectSuperFan == 1))){
				$this->TemplateHTML=str_replace("{ContentContent}",$ContentString,$this->TemplateHTML);
				} else {
				
					if ($this->AccessType == 'fans') 
							$Wanrning= 'images/fan_content_warning.jpg';
					else if ($this->AccessType == 'superfans') 
							$Wanrning= 'images/superfan_content_warning.jpg';

					$ContentString = '<div align="center"><img src="/'.$Wanrning.'"></div>';
		...
		function drawStyle() {
					...
					$this->PageAreaWidth = $SkinArray->PageAreaWidth;
					...
					echo '#pagediv{';
                         	if ($SkinArray->PageAreaImage != '') {
                                    list($Width,$Height)=@getimagesize(PFENGINEDIR.'/templates/skins/'.$this->SkinCode.'/images/'.$SkinArray->PageAreaImage);
                           			 echo 'background-image:url(/templates/skins/'.$this->SkinCode.'/images/'.$SkinArray->PageAreaImage.');
                            		background-repeat:'.$PageAreaImageRepeat.';'; 
							}
                            echo 'background-color:#'.$SkinArray->PageAreaBGColor.';'; 
							
							if ($SkinArray->PageAreaRound == 1) {
								echo '-moz-border-radius-bottomright: '.$SkinArray->PageAreaCornerRadius.'px;
									   border-bottom-right-radius: '.$SkinArray->PageAreaCornerRadius.'px;';
									   
								echo '-moz-border-radius-bottomleft: '.$SkinArray->PageAreaCornerRadius.'px;
									   border-bottom-left-radius: '.$SkinArray->PageAreaCornerRadius.'px;';
									 
								echo '-moz-border-radius-topleft: '.$SkinArray->PageAreaCornerRadius.'px;
									   border-top-left-radius: '.$SkinArray->PageAreaCornerRadius.'px;';
									 
								echo '-moz-border-radius-topright: '.$SkinArray->PageAreaCornerRadius.'px;
									   border-top-right-radius: '.$SkinArray->PageAreaCornerRadius.'px;';
							}
							if ($SkinArray->PageAreaPadding != '') {
								echo 'padding:'.$SkinArray->PageAreaPadding.'px;';
							}
					echo '}';
		...
		public function build_menus($theme,$project,$custom,$MenuLayout) {
					...
					if ($custom == 0) {
						...
						$query = "select * from menu_links where ComicID='$project' and MenuParent=1 and  IsPublished=1 ORDER BY Parent, Position ASC";
					...
					while ($line = $db->fetchNextObject()) { 
						...
						$ButtonRollOver =  $line->RolloverButtonImage;
						$Url = $line->Url;
						if (($line->AccessType == 'public') || (($line->AccessType == 'fans') && (($this->IsFollowing == 1)||($this->IsProjectSuperFan == 1)))|| (($line->AccessType == 'superfans') && ($this->IsProjectSuperFan == 1))){
						$String .= '<td>';
						...
						if ($Button != '') {
							...
							if ($ButtonRollOver != '') 
								$LinkStuff .= ' onMouseOver="this.src=\''.$MenuImageDir.'/images/'.$ButtonRollOver.'\';"onMouseOut="this.src=\''.$MenuImageDir.'/images/'.$Button.'\';"';
							$LinkStuff .= '/>'; 
						} else {
