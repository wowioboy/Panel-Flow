	...
	class project {
		...
		//	echo 'QUERY = ' .  $query;
			...
			$this->MobileCount=  $db->queryUniqueValue($query);
				   	...
				   	//$query = "SELECT count(*) as InRumble, round_one_rank, round_two_rank, round_three_rank,in_lord from rumble_entries where project_id='".$this->ProjectID."' and (round_one='1' or round_two='1' or round_three='1') and active='1' group by project_id";
					
					/*
				   	$query = "SELECT count(*) as InRumble, r.round_one_rank, r.round_two_rank, r.round_three_rank,r.in_lord from rumble_entries as r where r.project_id='".$this->ProjectID."' and (r.round_one_rank=1 or r.r2_w1_rank=1 or r.r2_w2_rank=1 or r.r2_w3_rank=1 or r.r2_w4_rank=1 or r.r3_w1_rank=1 or r.r3_w2_rank=1 or r.r3_w3_rank=1 or r.r3_w4_rank=1) and r.active='1' group by r.project_id";
					
					
				$this->RumbleArray=  $db->queryUniqueObject($query);
				$this->InRumble = $this->RumbleArray->InRumble;
				$this->round_one_rank=  $this->RumbleArray->round_one_rank;
				$this->round_two_rank= $this->RumbleArray->round_two_rank;
				$this->round_three_rank=  $this->RumbleArray->round_three_rank;
				$this->in_lord=  $this->RumbleArray->in_lord;
	*/
			...
			$this->ProjectArray = array (
									...
									'cover'=>$ProjectTableArray->cover,
									'Rating'=>$ProjectTableArray->Rating,
									'Ranking'=>$ProjectTableArray->Ranking,
									'MobileCount'=>$this->MobileCount,
									'InRumble'=>$this->InRumble,
									'InLord'=>$this->in_lord,
									'RoundOneRank'=>$this->round_one_rank,
									'RoundTwoRank'=>$this->round_two_rank,
									'RoundThreeRank'=>$this->round_three_rank,
									'redirect'=>$ProjectTableArray->redirect
								);
			$query = "select count(*) from pf_subscriptions as s where s.UserID='".$this->ProjectArray['AdminUserID']."' and s.Status='active' and s.SubscriptionType='hosted_no_ads' and ((s.Temp=0) or ((s.Temp=1) and (s.Expires>=NOW())))"; 
			$this->IsProProject = $db->queryUniqueValue($query);
		 ...
		 public function getProjectRanking() {
								$db = new DB(PANELDB, PANELDBHOST, PANELDBUSER, PANELDBPASS);
								$today = date("Ymd"); 
								$query = "SELECT p.hits as TotalHits,p.Ranking as Rank, p.PagesUpdated as LastUpdated, p.cover, (select count(*) from favorites as f where f.ProjectID=p.ProjectID) as TotalVolts,
