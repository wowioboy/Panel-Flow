...
$query = "SELECT b.Title as BoardTitle, b.EncryptID as BoardEnc, b.PrivacySetting as BoardPrivacy, b.OpenTopics, u.avatar, u.username, t.*, c.Title as CatTitle, (SELECT count(*) FROM pf_forum_messages as m WHERE m.TopicID = t.ID) AS TotalReplies, u2.avatar as TopicThumb, u2.username as TopicUser
		  		from pf_forum_topics as t
				join pf_forum_boards as b on b.ID='$BoardID'
				left join users as u on t.LastUser=u.encryptid
				left join users as u2 on t.PosterID=u2.encryptid
				left join pf_forum_categories as c on c.ID=b.CatID 
...
$Auth = 0;
				...
				if ($Auth == 1) {	
					...
					$query = "SELECT m.*, u.username, u.avatar, u.ForumSig,
							  ...
							  from pf_forum_messages as m 
							  join users as u on m.PosterID = u.encryptid where m.TopicID='$TopicID' order by m.ID ASC";	
							$pagination->createPaging($query,$NumItemsPerPage,$querystring);
...
function PostTopic($BoardID, $Subject, $Message, $Sticky, $Locked, $PosterID, $UserArray, $ProjectArray,$PrivacySetting,$Tags, $Notify, $SelectedGroups='') {
				...
				if ($PosterID != '') {
				$query = "INSERT into pf_forum_topics (Subject, ProjectID, BoardID, UserID, PosterID, IsSticky, PrivacySetting, Tags, SelectedGroups, NotifyOnReply, Message, IsLocked, CreatedDate, LastReply, LastUser) 
							values (
							...
							$Locked,'$NOW',
							'".mysql_real_escape_string($PosterID)."','$NOW')";
