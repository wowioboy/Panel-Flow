...
if (!isset($_POST['currentpass'])) {  
 ...
 $error = "Please enter your current password and new password below";
	...
	$logresult = file_get_contents('http://www.wevolt.com/processing/pfusers.php?action=logincrypt&email='.$_SESSION['email'].'&pass='.md5($_POST['currentpass']));
   if (trim($logresult) != 'Not Logged') {
	$changeresult = file_get_contents ('http://www.wevolt.com/processing/pfusers.php?action=changepass&id='.trim($_SESSION['userid']).'&n='.$_POST['newpass']);
