...
$where = '';
	...
	$JOIN .= " JOIN pf_subscriptions as ps on ((p.userid=ps.UserID or p.CreatorID=ps.UserID) and ps.SubscriptionType='hosted' and ps.Status='active')";
...
if ((!isset($_GET['sub']))  && ($_GET['content'] != 'user')){
	...
	while ($line = $DB->fetchNextObject()) {
	 ...
	 $DB->close();
 if (($Results == 0) && ($_GET['section'] == 'shares'))
	$SearchString .=  "<div align='center' style='font-weight:bold;'>There are no PRO projects that  fit your search.</div>";
 else if ($Results == 0)
