...
if ($_POST['fileurl'] != '') {
	...
	if(move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
		...
		if (exif_imagetype($target_path) == IMAGETYPE_GIF) {
			...
			$Realext = substr(strrchr($Filename, "."), 1);	
		...
		//print 'Real EXT = ' . $Realext.'<br/>';
		
		$randName = md5(rand() * time());
		
		if ($Convert == 1){
			$convertString = "convert temp/$Filename -quality 100 temp/".$randName.".".$ext;
			exec($convertString);
			$Filename = $randName.'.'.$ext;
			chmod('temp/'.$Filename,0777);
		...
		if (($ext == 'jpg') || ($ext == 'gif') || ($ext == 'png')){
				...
				if ($ext == 'pdf') 
					$ThumbName =  'pdf_thumb.jpg';
				 else if ($ext == 'cbr') 
					$ThumbName =  'cbr_thumb.jpg';
				else
					$ThumbName =  'file_thumb.jpg';
		}
		?>
		...
		<? } else {?>
					...
					<? } else {?>
						...
						<? } else {
							...
							?>
						window.parent.document.getElementById("pageimage").src = '/images/temps/<? echo $ThumbName;?>';	
						window.parent.document.getElementById("txtThumb").value = '<? echo $ThumbName;?>';	
