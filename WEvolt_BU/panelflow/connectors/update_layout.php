...
if (($UserID == $ComicArray->userid) || ($UserID == $ComicArray->CreatorID) || (in_array($UserID,$AssistantArray))) {
	...
	$updateresult = $curl->send_post_data("https://www.panelflow.com/connectors/update_layout.php", $post_data);
	unset($post_data);
	echo 'UPDATE RESULT = '.$updateresult.'<br/>';
			if ($updateresult != 'Not Authorized') {
				...
				$LayoutHTML =mysql_real_escape_string($values['LayoutHTML']);
				
				$query = "UPDATE comic_settings SET LayoutHTML='$LayoutHTML', LayoutType='$LayoutType' where ComicID='$ComicID'";
