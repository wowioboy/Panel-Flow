...
//GRAB PAGE
	...
	if ($Action == 'add') {
		$query = "INSERT into comic_pages(ComicID, Title, Comment, Image, ImageDimensions, Datelive, ThumbSm, ThumbMd, ThumbLg, Chapter, Episode, Filename, Position, UploadedBy,EncryptPageID, PageType) values ('$ComicID','$Title', '$Comment','$Filename','$ImageDimensions', '$Datelive','$NewThumbsm','$NewThumbmd','$NewThumblg',$Chapter,$Episode,'$Filename',$ItemPosition,'$UserID','$PageID','$PageType')";
			...
			if ($Section == 'pages') {
				...
				$query = "SELECT Position from comic_pages WHERE Position=(SELECT MAX(Position) FROM comic_pages where ComicID='$ComicID' and PageType = 'extras')";
			}
			...
			if ($NewItemPosition != $CurrentPosition) {
				$CurrentOrder = array();
				if ($NewItemPosition < $CurrentPosition) {
					$query = "SELECT EncryptPageID, Position from $DbTable where ComicID='$ComicID' and PageType='$PageType' and Position BETWEEN '$NewItemPosition' and '$CurrentPosition' order by Position";
				} else {
					$query = "SELECT EncryptPageID, Position from $DbTable where ComicID='$ComicID' and PageType='$PageType' and Position BETWEEN '$CurrentPosition' and '$NewItemPosition' order by Position";
				}
				$settings->query($query);
		...
		if ($Section == 'pages') {
			...
			$query = "SELECT Position from extra_pages WHERE Position=(SELECT MAX(Position) FROM comic_pages where ComicID='$ComicID' and PageType='$PageType')";
	}
		$MaxPosition = $settings->queryUniqueValue($query);
	//print $query."<br/>";
