...
if ($_POST['fileurl'] != '') {
	...
	$gif = file_get_contents($Image) or die('Could not grab the file');
			...
			<? } else {?>
				...
				window.parent.document.getElementById("pageimage").src = '/pf_16_core/temp/thumbs/<? echo $Filename;?>';
			...
			<? }?>
				...
				<? if (isset($_GET['type'])) {?>
					document.location.href = 'includes/file_upload_inc.php?type=<? echo $_GET['type'];?>&compact=<? echo $_GET['compact'];?>';
				<? } else { ?>
					document.location.href = 'includes/file_upload_inc.php?compact=<? echo $_GET['compact'];?>';
				<? }?> 
...
<script type="text/javascript">
		...
		<? if ($_GET['type'] != '') {
				if ($_GET['type'] =='pencils') { ?>
					window.parent.document.getElementById("txtPeelOneFilename").value = '<? echo $Filename;?>';
					window.parent.document.getElementById("pencilsdiv").innerHTML = '<img src=\"/pf_16_core/temp/thumbs/<? echo $Filename;?>\" id=\"peeloneimage\">';
					//window.parent.document.getElementById("peeloneimage").src = '/pf_16_core/temp/thumbs/<? //echo $Filename;?>';
				<? } else if ($_GET['type'] =='colors') { ?>
					window.parent.document.getElementById("txtPeelThreeFilename").value = '<? echo $Filename;?>';
					window.parent.document.getElementById("colorsdiv").innerHTML = '<img src=\"/pf_16_core/temp/thumbs/<? echo $Filename;?>\" id=\"peeltwoimage\">';
					//window.parent.document.getElementById("peelthreeimage").src = '/pf_16_core/temp/thumbs/<? //echo $Filename;?>';
				<? } else if ($_GET['type'] =='inks') { ?>
					window.parent.document.getElementById("txtPeelTwoFilename").value = '<? echo $Filename;?>';
					window.parent.document.getElementById("inksdiv").innerHTML = '<img src=\"/pf_16_core/temp/thumbs/<? echo $Filename;?>\" id=\"peelthreeimage\">';
					//window.parent.document.getElementById("peeltwoimage").src = '/pf_16_core/temp/thumbs/<? //echo $Filename;?>';
				<? } else if ($_GET['type'] =='store') { ?>
					window.parent.document.getElementById("txtThumb").value = '<? echo $Filename;?>';
					window.parent.document.getElementById("currentthumb").innerHTML = '<img src=\"/pf_16_core/temp/thumbs/<? echo $Filename;?>\">';
	...
	<? } ?>
    	...
    	echo "There was an error uploading the file, please try again!<br/><input type='button' onclick='document.location.href=\"includes/file_upload_inc.php?compact=".$_GET['compact']."&type=".$_GET['type']."\";' value='GO BACK'>";
	}
