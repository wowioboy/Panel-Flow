...
$query ="INSERT into pf_store_items (ComicID, UserID, ShortTitle, Description, Tags, ShippingOption,ProductType,DownloadFile,Price) values('$ComicID','".$_SESSION['userid']."','$Title','$Subject','$Keywords',1,'pdf','$PDFFILE','$Price')";
$PDFDB->execute($query);

$query ="SELECT ID from pf_store_items where DownloadFile='$PDFFILE'";
$ProductID = $PDFDB->queryUniqueValue($query);

$Encryptid = substr(md5($ProductID), 0, 8).dechex($ProductID);

$query = "UPDATE pf_store_items SET EncryptID='$Encryptid' WHERE ID='$ProductID'";
$PDFDB->execute($query);

$query = "INSERT into pf_store_images (ItemID, ThumbSm, ThumbMd, ThumbLg, IsMain) values ('$Encryptid','$Thumbsm','$Thumbmd','$Thumblg',1)";
$PDFDB->execute($query);
...
$ActionString = '&sub=pdf&a=finish&pid='.$Encryptid;
} else {
$EncryptID = $_POST['txtProductID'];
$ActionString = '&sub=pdf&a=edit&pid='.$EncryptID;
$query ="UPDATE pf_store_items set ShortTitle='$Title', Description='$Subject', Tags='$Keywords', DownloadFile='$PDFFILE',Price='$Price' where EncryptID='$EncryptID'";
$PDFDB->execute($query);

$query ="UPDATE pf_store_images set ThumbSm='$Thumbsm', ThumbMd='$Thumbmd', ThumbLg='$Thumblg'
 where ItemID='$EncryptID' and IsMain=1";
$PDFDB->execute($query);
...
$query = "SELECT * from pf_store_items where EncryptID ='".$_POST['txtProductID']."'";
...
$query = "SELECT * from products_attributes_pdf where ProductID ='".$_POST['txtProductID']."'";
...
if (((isset($_POST['creditscomplete']))  || ($_POST['txtInfoClicked'] == 1)) && ($UserInfoComplete == 1)) {
header("Location:/cms/edit/".$_GET['comic']."/?section=products".$ActionString);
}
...
<meta name="description" content="Web Comic Content Management System"></meta>
...
<body background="/<? echo $PFDIRECTORY;?>/images/admin_bg.jpg" style="background-repeat:repeat-x;">
...
<form method="post" action="/process/pdf/<? echo $SafeFolder;?>/" name="InfoForm" id="InfoForm">
...
<? } else { ?>
<div align="center"> 
<form method="post" action="/process/pdf/<? echo $_GET['comic'];?>/" name="CreditForm" id="CreditForm">
<div class="pageheader" style="padding-left:300px;"> Enter your PDF's Credits/Information</div>
<div class="spacer"></div>
<div  style="padding-right:30px;">
When you are finished click here -> <input type="button" value="CREATE PDF" onClick="finish_pdf();" style="background-color:#FF9900;"/></div>
