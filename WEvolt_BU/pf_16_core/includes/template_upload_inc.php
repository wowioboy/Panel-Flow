...
function submit_form() {
		...
		document.UploadForm.action = '../theme_uploader.php?type=<? echo $_GET['type'];?>&skincode=<? echo $_GET['skincode'];?>&comic=<? echo $_GET['comic'];?>&db=<? echo $_GET['db'];?>&template=<? echo $_GET['template']?>&project=<? echo $_GET['project'];?>&theme=<? echo $_GET['theme'];?>';
...
<? if ($_GET['vert'] != '1') {?><table><tr><td><? }?>
<input name="uploadedfile" type="file" style="width:200px;" id="uploadedfile"/>
<? if ($_GET['vert'] != '1') {?></td><td valign="top" style="padding-left:5px;"><? }?><input type="button" value="UPLOAD" onClick='submit_form();'/>
<input type="hidden" value="<? echo $_GET['bg'];?>" name="bg" />
<input type="hidden" value="<? echo $_GET['compact'];?>" name="compact" />
<input type="hidden" value="<? echo $_GET['transparent'];?>" name="transparent" /><? if ($_GET['vert'] != '1') {?></td></tr></table><? }?>
</form>
</div>
