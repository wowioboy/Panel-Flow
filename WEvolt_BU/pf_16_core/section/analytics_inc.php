...
if (($_POST['m'] != $CurrentMonth) &&($_POST['m'] != '')) {
	...
	$TargetID = 'ProjectID';
	$UNIONVIEWS = " UNION SELECT count(remote) as PageViewsViews, AnalyticDate  from panel_analytics.current_month where (comicid='$ComicID' or ProjectID='$ComicID') and AnalyticDate>='".date('Y-m')."-01 00:00:00'";
	$UNIQUEUNIONVIEWS = " UNION SELECT count(distinct remote) as PageViewsViews, AnalyticDate from panel_analytics.current_month where (comicid='$ComicID' or ProjectID='$ComicID') and AnalyticDate>='".date('Y-m')."-01 00:00:00'";
	$UNIONLOCATIONS = " UNION SELECT distinct Latitude, Longitude FROM panel_analytics.current_month where $TargetID='$ComicID' and Latitude != '' and Longitude != '' and AnalyticDate>='".date('Y-m')."-01 00:00:00'";
	$UNIONREFERAL = " UNION SELECT count(Referal) as Total, Referal FROM panel_analytics.current_month where $TargetID='$ComicID' and Referal !='$Domain' and AnalyticDate>='".date('Y-m')."-01 00:00:00'";
...
if ($_POST['m'] =='00')
   ...
   flush(); 
	 ...
	 $query = "SELECT count(remote) as PageViewsViews, AnalyticDate  
				FROM ".$ViewsTable." where (comicid='$ComicID' or ProjectID='$ComicID') ".$DateQuery."";
	//$query .=$UNIONVIEWS;
	$query .= " group by AnalyticDate order by AnalyticDate desc";
	 ...
	 $graphLabels = "";
	  for ($i=0; $i< $nRows; $i++){
		  ...
		  $graphLabels .= date('m-d-Y',strtotime($analytics['AnalyticDate']));
		 //$graphLabels .= $analytics['date'];

		    $Day = date('d',strtotime($analytics['AnalyticDate']));
		   $Month = date('m',strtotime($analytics['AnalyticDate']));
		   $Lastweek = $Day - 7;
		   $LastMonth = $Month - 1;
		   if ($Lastweek < 1) {
		   ...
		   if ($analytics['AnalyticDate'] == $today)  {  
		  	 $TodayHits = $analytics['PageViewsViews'];
		   }
		  		 
	      }
		  
		  $query = "SELECT count(remote) as PageViewsViews, AnalyticDate from panel_analytics.current_month where (comicid='$ComicID' or ProjectID='$ComicID') and AnalyticDate>='".date('Y-m')."-01 00:00:00'";
	//$query .=$UNIONVIEWS;
	$query .= " group by AnalyticDate order by AnalyticDate desc";
	 $result = mysql_query($query);
	 ...
	 $noRows = @mysql_num_rows($result);
	  for ($i=0; $i< $noRows; $i++){
		 ...
		 $query = "SELECT count(distinct remote) as PageViewsViews, AnalyticDate  
				FROM ".$ViewsTable." where (comicid='$ComicID' or ProjectID='$ComicID') ".$DateQuery."";
	//$query .=$UNIQUEUNIONVIEWS;
	$query .= " group by AnalyticDate order by AnalyticDate desc";
	 ...
	 $uniquegraphLabels = "";
	  for ($i=0; $i< $nRows; $i++){
		  ...
		  $uniquegraphLabels .= ',';
		   ...
		   $uniquegraphValues .= $analytics['PageViewsViews'];
		   $uniquegraphLabels .= date('m-d-Y',strtotime($analytics['AnalyticDate']));
		   if ($analytics['AnalyticDate'] == $today)  {  
		  	 $TodayUniqueHits = $analytics['PageViewsViews'];
		   }
		   $Day = date('d',strtotime($analytics['AnalyticDate']));
		   $Month = date('m',strtotime($analytics['AnalyticDate']));
		   $Lastweek = $Day - 7;
		   ...
		   $query = "SELECT count(distinct remote) as PageViewsViews, AnalyticDate from panel_analytics.current_month where (comicid='$ComicID' or ProjectID='$ComicID') and AnalyticDate>='".date('Y-m')."-01 00:00:00'";
	$query .= " group by AnalyticDate order by AnalyticDate desc";
	...
	$uniquegraph->absValuesBorder = "2px groove white";
	 ...
	 $query = "SELECT count(Referal) as Total, Referal FROM ".$ViewsTable." where (comicid='$ComicID' or ProjectID='$ComicID') and Referal !='$Domain' ".$DateQuery." ";
	// $query .= $UNIONREFERAL;
	 $query .=" group by Referal order by Referal desc";
		
	
	 $result = mysql_query($query);
	 $nlRows = @mysql_num_rows($result);
	 $RefString = "";
	 
	  for ($i=0; $i< $nlRows; $i++){
   	      $referal = mysql_fetch_array($result);
			
		  $ReferalNameArray= explode('/',$referal['Referal']);
			$Referal = $ReferalNameArray[0];
			
   		    if (($Referal != $_SERVER['SERVER_NAME']) && ($Referal != '') && ($ReferalNameArray[1] != $_SESSION['safefolder'])){
				//$RefArray[] = array($analytics['Referal'],1);
				$RefString .= '<div>'.$referal['Referal'].' ('.$referal['Total'].')</div>';	
							
			}
		 }
		 
		  $query = "SELECT count(Referal) as Total, Referal FROM panel_analytics.current_month where $TargetID='$ComicID' and Referal !='$Domain' and AnalyticDate>='".date('Y-m')."-01 00:00:00'";
	// $query .= $UNIONREFERAL;
	 $query .=" group by Referal order by Referal desc";
...
foreach($RefArray as $ref) {
	$RefString .= '<div>'.$ref.' ('.$RefTotal[$n].')</div>';	
	$n++;
}
*/
?>
 ...
 <table width="95%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <div class="spacer"></div>
     ...
     Year: <select name="y">
     <option value="2011" <? if (($_POST['y'] == '2011') || (!isset($_POST['y']))) echo 'selected';?>>2011</option>
     <option value="2010" <? if ($_POST['y'] == '2010') echo 'selected';?>>2010</option>
      <option value="2009" <? if ($_POST['y'] == '2009') echo 'selected';?>>2009</option>
