...
if ((!isset($_GET['comic'])) && (!isset($_POST['txtUrl'])) && ($_GET['a'] != 'new')) {
    ...
    <?
		...
		if ($_GET['a'] == 'all') {
		$query = "SELECT Image from comic_pages where ComicID ='$ComicID' order by Position,PageType ASC";
		} else {
			$query = "SELECT Image from comic_pages where ComicID ='$ComicID' and PageType='pages' order by Position ASC";
...
<? }else {?>
	<div id='finishdiv' class="stepheader" style="padding-left:30px;padding-right:30px;padding-top:5px;<? if ($_GET['a']=='new') echo 'display:none;';?>" align="center">
<? }?>
...
<?php
	...
	echo '<ul id="boxes">';
		...
		foreach ($images_url_array as $Image) {
			...
			$pdfDB =  new DB($db_database,$db_host, $db_user, $db_pass);
			$query = "INSERT into products_incomplete (ComicID,UserID, TotalPages, TempDirectory, ThumbList, PageList) values ('$ComicID','".$_SESSION['userid']."','$Count','$randDirectory','$ThumbString;','$NewPages')";
			$pdfDB->query($query);
			$pdfDB->close();
			}
			?>
...
<div style="clear: left;"><br/></div>
 <div id='uploaddiv' <? if ($_GET['a']!='new') {?>style="display:none;"<? }?> align="center">
